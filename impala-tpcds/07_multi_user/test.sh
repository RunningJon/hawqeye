...
for i in $(ls $sql_dir/*.sql); do
	...
	while [ "$run_query" -eq "1" ]; do
		...
		if [ "$oom_count" -gt "0" ]; then
			...
			if [[ "$error_state_store_count" -gt "0" || "$error_connect_timeout_count" -gt "0" || "$error_communicate_impalad_count" -gt "0" || "$error_connection_reset_count" -gt "0" || "$error_connection_refused_count" -gt "0" || "$warning_unreachable_impalad" -gt "0" || "$error_econnreset" -gt "0" || "$error_count" -gt "0" ]]; then
